...
class Trainer(nn.Module):
    ...
    def train_on_batch(self, *batch, device):
        ...
        x_batch = torch.as_tensor(x_batch, device=device, dtype=torch.float32)
        y_batch = torch.as_tensor(y_batch, device=device, dtype=torch.float32)
    ...
    def evaluate_mse(self, X_test, y_test, device, batch_size=4096):
        X_test = torch.as_tensor(X_test, device=device, dtype=torch.float32)
        y_test = check_numpy(y_test)
        self.model.train(False)
        with torch.no_grad():
    ...
    def get_prediction(self, X_test, y_test, device, batch_size=4096):
        X_test = torch.as_tensor(X_test, device=device, dtype=torch.float32)
        y_test = check_numpy(y_test)
        self.model.train(False)
        with torch.no_grad():
